...
const ContactSchema = new mongoose.Schema({
  account_id: {
    ...
    ref: "Account",
    required: function () { return !this.is_template; },
  },
  chatbot_id:  { type: mongoose.Schema.Types.ObjectId, ref: "Chatbot" },
  session_id:  { type: mongoose.Schema.Types.ObjectId, ref: "ConversationSession", sparse: true },
  source:      { type: String, enum: ["chatbot", "manual", "system"], default: "chatbot" },
  ...
  completed_goal:   { type: Boolean, default: false },
  lead_score:       { type: Number,  default: 0 },
  status_changed_at:  { type: Date, default: null },
  lost_limit_at:      { type: Date, default: null },
  discarded_limit_at: { type: Date, default: null },
  discarded_reason:   { type: String, default: null },
  discarded_notes:    { type: String, default: null },
  duration_seconds:   Number

}, { timestamps: true });

// ── Índices de lectura general ────────────────────────────────────────────────
ContactSchema.index({ account_id: 1, createdAt: -1 });
ContactSchema.index({ account_id: 1, lead_score: -1 });
ContactSchema.index({ account_id: 1, visitor_id: 1, is_deleted: 1 });

// ── Índices para filtros combinados frecuentes ────────────────────────────────
ContactSchema.index({ account_id: 1, status: 1, createdAt: -1 });
