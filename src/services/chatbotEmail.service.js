...
<body style="margin:0;padding:0;background:#F2F2F2;font-family:Arial,Helvetica,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:30px 0;">
      ...
      <table width="600" cellpadding="0" cellspacing="0"
        ...
        <tr>
          <td style="background:#034AA6;padding:22px 28px;">
            ...
            <p style="margin:5px 0 0;color:#04C4D9;font-size:12px;letter-spacing:1px;text-transform:uppercase;">
    ...
    const { data, error } = await resend.emails.send({
      from:    `${emailSettings.from_name || "Chatbot"} <onboarding@resend.dev>`,
      to:      toEmails,
    ...
    if (error) {
      console.error("❌ Error Resend (chatbot email):", error);
    } else {
      console.log(`📧 Email enviado a ${toEmails.length} destinatario(s): ${toEmails.join(", ")} | id: ${data?.id}`);
    }
